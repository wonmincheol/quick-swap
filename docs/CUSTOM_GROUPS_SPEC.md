...
팝업은 설정할 때만 필요하다. 실제 전환은 팝업을 닫은 상태에서 동작한다.
...
신규 슬롯 선택과 전환 후보에 동일한 지원 조건을 적용한다.
저장 검증은 좌표와 중복을 확인하며, 콘텐츠 변경으로 사라지거나 지원 대상에서 제외된 기존 항목은 보존한다.
...
- 설정에 스키마 버전 1을 저장한다. 현재 0.3.x 내에서 구조 변경은 없으며, 향후 구조 변경 시 마이그레이션을 추가한다.
...
| `scripts/belt_catalog.lua` | 기본 그룹에 사용할 프로토타입 후보 정렬 |
| `scripts/groups.lua` | 개인 설정, 기본 그룹 생성, 검증, 좌표 색인과 배열 탐색 |
| `scripts/gui.lua` | 상단 버튼, 팝업, 임시 편집, 적용·취소 |
| `scripts/unlocks.lua` | 세력별 해금 목록과 갱신 처리 |
